...
\begin{tikzpicture}[remember picture,overlay]
  ...
  \node [yshift=-0.75cm, xshift=1cm, anchor=west] at (current page.north west)
      {\color{gray!10}\fontsize{0.6cm}{1em}
        {\bf REVISTA SOLUÇÕES}
        };

  \node [yshift=3.25cm] at (current page.center)
      {\color{redsol}\fontsize{1.2cm}{1em}
        \includegraphics[scale=1.2]{SaemaLog.png}
        };
\end{tikzpicture}
...
\begin{tikzpicture}[remember picture,overlay]
  ...
  \node [yshift=7.5cm, xshift=1cm,
    ...
    {\color{white}
      ...
      \node [yshift=1.3cm] at (current page.south)
      {\color{white}
        \begin{tabular}[l]{c}
